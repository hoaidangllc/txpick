...
export default function Summary() {
  ...
  return <div className="container-app py-6 sm:py-8"><div className="flex items-end justify-between gap-3"><div><h1 className="text-3xl font-extrabold text-ink-900 flex items-center gap-2"><FileText className="w-7 h-7 text-brand-600" /> {ui.summary.title}</h1><p className="text-ink-500 mt-1">{ui.summary.subtitle}</p></div><button onClick={exportCSV} className="btn-primary"><Download className="w-4 h-4" /> {ui.summary.export}</button></div><section className="mt-5 grid sm:grid-cols-3 gap-3"><StatCard label={ui.summary.yearExpenses} value={fmtUSD(yearly)} icon={Wallet} tone="brand" /><StatCard label={ui.summary.monthlyBills} value={fmtUSD(bills.reduce((s, b) => s + Number(b.amount || 0), 0))} /><StatCard label={ui.summary.records} value={expenses.length} /></section><div className="mt-5 card overflow-hidden"><table className="w-full text-sm"><thead className="bg-ink-50 text-ink-500"><tr><th className="px-4 py-3 text-left">{ui.summary.month}</th><th className="px-4 py-3 text-right">{ui.summary.business}</th><th className="px-4 py-3 text-right">{ui.summary.personal}</th><th className="px-4 py-3 text-right">{ui.summary.total}</th></tr></thead><tbody className="divide-y divide-ink-100">{rows.length === 0 ? <tr><td colSpan="4" className="py-10 text-center text-ink-400">{ui.summary.empty}</td></tr> : rows.map(([m, v]) => <tr key={m}><td className="px-4 py-3 font-semibold text-ink-900">{m}</td><td className="px-4 py-3 text-right">{fmtUSD(v.business)}</td><td className="px-4 py-3 text-right">{fmtUSD(v.personal)}</td><td className="px-4 py-3 text-right font-bold">{fmtUSD(v.total)}</td></tr>)}</tbody></table></div><div className="mt-4 rounded-2xl bg-gold-500/10 text-gold-700 p-4 text-sm">{ui.summary.proNote}</div></div>
}
